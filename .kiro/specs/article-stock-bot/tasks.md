...
- [ ] 2.1 Discord Bot クライアントとイベントリスナーの実装
...
- [ ] 6.2 (P) ファイル名サニタイゼーションの実装
  - タイトルに使用できない文字（`/ \ : * ? " < > |`）を除去
  - ファイル名の最大長を100文字に制限し、超過分を切り詰め
  - _Requirements: 5.6, 5.7_

- [ ] 6.3 (P) ローカルストレージ処理の実装
...
- [ ] 11.1 (P) ユニットテスト: ContentParser
  - URL単体、URL+コメント、テキストのみの各パターンのテスト
  - 複数URL検出時の最初のURL抽出確認
  - 不正なURL形式の除外確認
  - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5_

- [ ] 11.2 (P) ユニットテスト: OGPScraper
  - OGP正常取得ケースのテスト
  - OGP不在時のフォールバック動作確認
...
- [ ] 11.4 (P) ユニットテスト: MarkdownGenerator
  - YAMLフロントマター生成確認
  - ファイル名サニタイゼーション確認
  - 最大100文字制限の動作確認
  - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5, 5.6, 5.7_

- [ ] 11.5 (P) ユニットテスト: VaultStorage
  - 既存ファイルへのコメント追記確認
  - 日付フォーマット確認
  - ファイル不在時のエラーハンドリング確認
...
- [ ] 11.6 (P) 統合テスト: メッセージ受信からファイル生成までのフロー
